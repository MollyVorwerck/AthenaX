...
class TestPatchRunner:
    ...
    def validate(self):
        LOG.info('Validating checkstyles and dependency')
        proc = Popen([
            '/bin/bash', '-c',
            'mvn clean checkstyle:check enforcer:enforce apache-rat:check --batch-mode --fail-at-end|tee ' + TARGET_DIR + '/validation.txt;'
    ...
    def compile(self):
        ...
        proc = Popen([
            '/bin/bash', '-c',
            'mvn compile test-compile enforcer:enforce'
    ...
    def run(self):
        ...
        ret = self.validate()
        if ret != 0:
            self.out.write('The validation of checkstyles / dependency have failed.\n')
            return ret
...
with open(TARGET_DIR + '/test-patch-result', 'w') as f:
    runner = TestPatchRunner(f)
    ret = runner.run()
    sys.exit(ret)
